...
validators = [
    ...
    Validator(
        "fileflows.api_key", must_exist=True, default="", is_type_of=str, cast=str
    ),
    Validator(
        "fileflows.workflow_id", must_exist=True, default="", is_type_of=str, cast=str
    ),
    Validator(
        "fileflows.timeout", must_exist=True, default=3600, is_type_of=int, gte=1
    ),
    Validator(
        "fileflows.loglevel",
        must_exist=True,
    ...
    Validator(
        "series_scores.release_group", must_exist=True, default=14, is_type_of=int
    ),
    ...
    Validator(
        "series_scores.streaming_service", must_exist=True, default=1, is_type_of=int
    ),
    Validator(
        "series_scores.hearing_impaired", must_exist=True, default=1, is_type_of=int
    ),
    ...
    Validator(
        "movie_scores.release_group", must_exist=True, default=13, is_type_of=int
    ),
    ...
    Validator(
        "movie_scores.streaming_service", must_exist=True, default=1, is_type_of=int
    ),
    Validator("movie_scores.edition", must_exist=True, default=1, is_type_of=int),
    Validator(
        "movie_scores.hearing_impaired", must_exist=True, default=1, is_type_of=int
    ),
    # postgresql section
...
def write_config():
    write(
        settings_path=config_yaml_file,
...
def save_settings(settings_items):
    ...
    if use_embedded_subs_changed or undefined_audio_track_default_changed:
        ...
        if settings.general.use_radarr:
            scheduler.add_job(
                list_missing_subtitles_movies, kwargs={"send_event": True}
            )
    ...
    if audio_tracks_parsing_changed:
        ...
        if settings.general.use_sonarr:
            ...
            scheduler.add_job(
                update_series, kwargs={"send_event": True}, max_instances=1
            )
        if settings.general.use_radarr:
            from radarr.sync.movies import update_movies

            scheduler.add_job(
                update_movies, kwargs={"send_event": True}, max_instances=1
            )
